...
`tools/loop_runner.py` 提供此人工觸發的有界執行器。它驗證前置條件（高風險操作立即轉為 `needs_human`）、在獨立循環中調用 maker 與 checker、寫入 `loop-state/state.json`，並在超過 iteration、時間、token 預算或達到重複失敗上限時硬性終止（`budget_exhausted` 或 `verification_failed`）。
